...
export default function Navbar({ currentPage, setCurrentPage }: NavbarProps) {
  ...
  return (
    <header id="sticky-header" className="sticky top-0 z-40 w-full border-b border-black/10 bg-cream/90 backdrop-blur-md transition-shadow duration-300">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`nav-underline-btn py-2 cursor-pointer focus:outline-none ${
      ...
      <div 
        className={`md:hidden overflow-hidden transition-all duration-300 border-b border-black/10 bg-cream ${
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.value}
                onClick={() => handleNavClick(item.value)}
                className={`text-left py-2 border-l-2 pl-3 transition-all duration-200 hover:text-gold active:translate-x-1 cursor-pointer ${
                  isActive
